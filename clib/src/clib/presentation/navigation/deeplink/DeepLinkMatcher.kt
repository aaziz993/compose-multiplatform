...
private const val TAG_LOG: String = "DeepLink"
...
internal class DeepLinkMatcher<T : NavKey>(
    ...
    fun match(): DeepLinkMatchResult<T>? {
        if (request.rawSegments.size != deepLinkPattern.pathSegments.size) return null
        // exact match (url does not contain any arguments)
        if (request.url == deepLinkPattern.urlPattern)
                ...
                if (candidateSegment.isParamArg) {
                    val parsedValue = try {
                        ...
                        nav3Logger.error(e, TAG_LOG) {
                            "Failed to parse path value:[$requestedSegment]."
                        }
                        return null
                    }
