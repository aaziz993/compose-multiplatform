 ...
 *
 * @param T the backstack key type that supports the deep linking of [urlPattern]
 * @param serializer the serializer of [T]
 * @param urlPattern the supported deeplink's uri pattern, i.e. "abc.com/home/{pathArg}"
 */
internal class DeepLinkPattern<T : NavKey>(
    ...
    val pathSegments: List<PathSegment> by lazy {
        urlPattern.rawSegments.map { segment ->
            ...
            if (result != null) {
                ...
                PathSegment(argName, true, getTypeParser(elementDescriptor.kind))
            }
            else
    ...
    val queryValueParsers: Map<String, TypeParser> =
        urlPattern.parameters.toMap().mapValues { (paramName, _) ->
 ...
 */
internal typealias TypeParser = (String) -> SerializableAny
