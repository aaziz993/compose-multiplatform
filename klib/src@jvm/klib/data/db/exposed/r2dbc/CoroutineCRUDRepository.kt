...
public class CoroutineCRUDRepository<T : Any>(
...
) : CoroutineCRUDRepository<T>, AbstractEntityRepository<T>() {
    ...
    override suspend fun <R> transactional(block: suspend CoroutineCRUDRepository<T>.(CoroutineTransaction) -> R): R =
        suspendTransaction(
            ...
            this@CoroutineCRUDRepository.statementCount?.let { statementCount = it }
            this@CoroutineCRUDRepository.duration?.let { duration = it }
            this@CoroutineCRUDRepository.warnLongQueriesDuration?.let { warnLongQueriesDuration = it }
            this@CoroutineCRUDRepository.debug?.let { debug = it }
            this@CoroutineCRUDRepository.maxAttempts?.let { maxAttempts = it }
            this@CoroutineCRUDRepository.minRetryDelay?.let { minRetryDelay = it }
            this@CoroutineCRUDRepository.maxRetryDelay?.let { maxRetryDelay = it }
            this@CoroutineCRUDRepository.queryTimeout?.let { queryTimeout = it }
            block(CoroutineTransaction(this))
        }
    ...
    private fun BooleanVariable.toPredicate(): Op<Boolean> =
        ...
        { expression, arguments ->
            LiteralOp(KotlinUuidColumnType(), Uuid.parse(""))
            when (expression) {
    ...
    public fun Value<*>.toLiteral() = when (this) {
        is Field -> table[value]!!
        is BooleanValue -> booleanLiteral(value)
        is UByteValue -> ubyteLiteral(value)
        is UShortValue -> ushortLiteral(value)
        is UIntValue -> uintLiteral(value)
        is ULongValue -> ulongLiteral(value)
        is ByteValue -> byteLiteral(value)
        is ShortValue -> shortLiteral(value)
        is IntValue -> intLiteral(value)
        is LongValue -> longLiteral(value)
        is FloatValue -> floatLiteral(value)
        is DoubleValue -> doubleLiteral(value)
        is CharValue -> stringLiteral(value.toString())
        is StringValue -> stringLiteral(value)
        is BigDecimalValue -> decimalLiteral(value.toJavaBigDecimal())
        is LocalTimeValue -> timeLiteral(value)
        is LocalDateValue -> dateLiteral(value)
        is LocalDateTimeValue -> dateTimeLiteral(value)
        is DurationValue -> durationLiteral(value)
        is UuidValue -> stringLiteral(value.toString())
