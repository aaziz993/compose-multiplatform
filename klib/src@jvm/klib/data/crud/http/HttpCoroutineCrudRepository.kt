...
public inline fun <reified T : Any> CoroutineCrudRepository<T>.routes(baseUrl: String) {
    routing.route(baseUrl) {
        routing.post("find") {
            with(call.receive<HttpCrud.Find>()) {
                call.respondFlow(
                    ...
                    typeInfo = typeInfo<T>(),
                    flow = find(predicate as BooleanOperand?, orderBy, limitOffset),
                )
            }
        }
        ...
        routing.post("findProperties") {
            with(call.receive<HttpCrud.FindProperties>()) {
                call.respondAnyFlow(
                    status = HttpStatusCode.OK,
                    flow = find(properties, predicate as BooleanOperand?, orderBy, limitOffset),
                )
            }
        }
        ...
        routing.post("observe") {
            with(call.receive<HttpCrud.Find>()) {
                call.respondFlow(
                    status = HttpStatusCode.OK,
                    typeInfo = typeInfo<T>(),
                    flow = observe(predicate as BooleanOperand?, orderBy, limitOffset),
                )
            }
        }
        ...
        routing.post("observeProperties") {
            with(call.receive<HttpCrud.FindProperties>()) {
                call.respondAnyFlow(
                    status = HttpStatusCode.OK,
                    flow = observe(properties, predicate as BooleanOperand?, orderBy, limitOffset),
                )
            }
        }
