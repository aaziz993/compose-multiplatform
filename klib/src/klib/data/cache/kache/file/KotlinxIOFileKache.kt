...
import klib.data.cache.kache.ContainerKache
...
/**
 * A persistent coroutine-safe [ContainerKache] implementation that uses [KotlinxIO] to
 * store files.
 *
 ...
 * ```
 * val cache = KotlinxIOFileKache(directory = "cache".toPath(), maxSize = 100L * 1024L * 1024L) {
 ...
 * @see Configuration
 */
public class KotlinxIOFileKache internal constructor(
    ...
    private val underlyingKache = InMemoryKache<String, String>(maxSize = maxSize) {
        ...
        this.sizeCalculator = { _, filename -> fileSystem.metadataOrNull(filesDirectory.resolve(filename))?.size ?: 0 }
        this.onEntryRemoved = { _, key, oldValue, newValue -> onEntryRemoved(key, oldValue, newValue) }
        this.creationScope = this@KotlinxIOFileKache.creationScope
    }
    ...
    private var journalWriter =
        JournalWriter(fileSystem.sink(journalFile, true).buffered())
    ...
    private suspend fun wrapCreationFunction(
        ...
        try {
            ...
            return if (creationFunction(tempFile) && fileSystem.exists(tempFile)) {
                fileSystem.atomicMove(tempFile, cleanFile, true)
    ...
    /**
     * Configuration for [KotlinxIOFileKache] used as a receiver for its builder function.
     */
    public class Configuration(
         ...
         */
        public var creationScope: CoroutineScope = CoroutineScope(StandardDispatchers.io)
...
/**
 * Creates a new instance of [KotlinxIOFileKache] with the given [directory], [maxSize] and [configuration].
 *
 ...
 */
public suspend fun KotlinxIOFileKache(
    directory: Path,
    maxSize: Long,
    ...
    return KotlinxIOFileKache.open(
