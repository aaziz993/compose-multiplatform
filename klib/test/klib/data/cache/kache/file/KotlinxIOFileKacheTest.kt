...
class KotlinxIOFileKacheTest {

    @Test
    fun getKeys() = runTest {
        val fileSystem = SystemFileSystem
        val kache = testKotlinxIOFileKache(fileSystem)
        ...
        // Re-open
        kache.close()
        val kache2 = testKotlinxIOFileKache(fileSystem)
    ...
    fun getOrPut() = runTest {
        ...
        val firstCachePath = kache.getOrPut(KEY_1) {
            fileSystem.sink(it).buffered().use { it.writeString(VAL_1) }
            true
        }
        fileSystem.assertPathContentEquals(VAL_1, firstCachePath)
        ...
        val secondCachePath = kache.getOrPut(KEY_2) {
            ...
            fileSystem.sink(it).buffered().use { it.writeString(VAL_2) }
            false
        }
        ...
        kache.putAsync(KEY_4) { cachePath ->
            delay(60_000)
            fileSystem.sink(cachePath).buffered().use { it.writeString(VAL_3) }
            false
        }
        val fourthCachePath = kache.getOrPut(KEY_4) {
            fileSystem.sink(it).buffered().use { it.writeString(VAL_4) }
            true
        }
    ...
    fun put() = runTest {
        ...
        val firstCachePath = kache.put(KEY_1) { cachePath ->
            fileSystem.sink(cachePath).buffered().use { it.writeString(VAL_1) }
            true
        }
        fileSystem.assertPathContentEquals(VAL_1, firstCachePath)
        ...
        val secondCachePath = kache.put(KEY_2) { cachePath ->
            ...
            fileSystem.sink(cachePath).buffered().use { it.writeString(VAL_2) }
            false
        }
        assertNull(secondCachePath)
        assertFalse(fileSystem.exists(capturedSecondCachePath!!))
        ...
        val thirdCachePath = kache.put(KEY_3) { cachePath ->
            fileSystem.sink(cachePath).buffered().use { it.writeString(VAL_4) }
            true
        }
    ...
    fun removeAllUnderCreation() = runTest {
        ...
        val deferred = kache.putAsync(KEY_3) { cachePath ->
            ...
            fileSystem.sink(cachePath).buffered().use {
                delay(60_000)
                it.writeString(VAL_3)
            }
            true
        }
        delay(1000)
    ...
    fun rebuildingJournal() = runTest {
        ...
        repeat((KotlinxIOFileKache.REDUNDANT_ENTRIES_THRESHOLD / 2) - 1) {
            kache.put(KEY_1) { false }
        }

        // The journal should not be rebuilt yet
        assertTrue((fileSystem.metadataOrNull(journalFile)?.size ?: 0) > 1024)
    ...
    fun reopen() = runTest {
        ...
        val contentFileSystem = SystemFileSystem
        createThreeElementKotlinxIOFileKacheWithAccess(contentFileSystem, KacheStrategy.LRU)
        val contentKache = testKotlinxIOFileKache(contentFileSystem)
        ...
        val lruFileSystem = SystemFileSystem
        createThreeElementKotlinxIOFileKacheWithAccess(lruFileSystem, KacheStrategy.LRU)
        val lruKache = testKotlinxIOFileKache(lruFileSystem, maxSize = 17L) { strategy = KacheStrategy.LRU }
        lruKache.put(lruFileSystem, KEY_4, VAL_4)
        assertNull(lruKache.get(KEY_3))

        // FIFO: The cache should keep the order of the entries
        val fifoFileSystem = SystemFileSystem
        createThreeElementKotlinxIOFileKacheWithAccess(fifoFileSystem, KacheStrategy.FIFO)
        val fifoKache = testKotlinxIOFileKache(fifoFileSystem, maxSize = 17L) { strategy = KacheStrategy.FIFO }
        ...
        val corruptedFileSystem = SystemFileSystem
        createThreeElementKotlinxIOFileKacheWithAccess(corruptedFileSystem, KacheStrategy.LRU)
        corruptedFileSystem.write(journalFile) { writeUtf8("corrupted") }
        val corruptedKache = testKotlinxIOFileKache(corruptedFileSystem)
    ...
    private suspend fun TestScope.testKotlinxIOFileKache(
        fileSystem: FileSystem = SystemFileSystem,
        maxSize: Long = MAX_SIZE,
        configuration: KotlinxIOFileKache.Configuration.() -> Unit = {}
    ) = KotlinxIOFileKache(directory, maxSize) {
        this.fileSystem = fileSystem
        this.creationScope = this@testKotlinxIOFileKache
        configuration()
    }

    private suspend fun KotlinxIOFileKache.put(fileSystem: FileSystem, key: String, value: String): Path? {
        return put(key) { cachePath ->
            fileSystem.sink(cachePath).buffered().use { it.writeString(value) }
            true
        }
    }

    private suspend fun KotlinxIOFileKache.putAsync(fileSystem: FileSystem, key: String, value: String): Deferred<Path?> {
     ...
     */
    private suspend fun TestScope.createThreeElementKotlinxIOFileKacheWithAccess(
    ...
    private fun FileSystem.assertPathContentEquals(content: String, path: Path?) {
        ...
        assertEquals(content, source(path).buffered().use(Source::readString))
    }
