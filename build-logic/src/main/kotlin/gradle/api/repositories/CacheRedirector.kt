...
import gradle.api.kotlin.targets.web.node.nodeJsEnv
import gradle.api.kotlin.targets.web.yarn.yarnEnv
import java.net.URI
...
internal object CacheRedirector {

    /**
     * Substitutes repositories in buildScript { } block.
     */
    @Suppress("UnstableApiUsage")
    fun applyTo(receiver: Settings) {
        logger.info("Redirecting repositories for settings")

        receiver.pluginManagement.repositories.redirect()
        receiver.dependencyResolutionManagement.repositories.redirect()
        receiver.buildscript.repositories.redirect()
    }

    fun applyTo(receiver: Project) {
        logger.info("Redirecting repositories for project")

        receiver.buildscript.repositories.redirect()
        receiver.repositories.redirect()
        receiver.overrideNativeCompilerDownloadUrl()
        receiver.configureYarnAndNodeRedirects()
        receiver.addCheckRepositoriesTask()
    }
}
...
/**
 * Native compiler download url override
 */
private fun Project.overrideNativeCompilerDownloadUrl() {
...
private fun Project.configureYarnAndNodeRedirects() =
    project.pluginManager.withPlugin("com.github.node-gradle.node") {
        yarnEnv.downloadBaseUrl.assign(URI(yarnEnv.downloadBaseUrl.get()).maybeRedirect().toString())

        nodeJsEnv.downloadBaseUrl.assign(URI(nodeJsEnv.downloadBaseUrl.get()).maybeRedirect().toString())
    }
...
private fun Project.logNonCachedRepo(
    testName: String,
    repoUrl: String,
) {
...
private fun Project.logInvalidIvyRepo(
    testName: String,
) {
...
private fun testStarted(ciName: String, testName: String) {
    println("##$ciName[testStarted name='%s']".format(escape(testName)))
}

private fun testFinished(ciName: String, testName: String) {
    println("##$ciName[testFinished name='%s']".format(escape(testName)))
}

private fun testFailed(ciName: String, testName: String, message: String, details: String) {
    println(
        "##$ciName[testFailed name='%s' message='%s' details='%s']".format(
            escape(testName),
            escape(message),
            escape(details),
        ),
