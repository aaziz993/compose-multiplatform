...
public class SigningPlugin : Plugin<Project> {
    ...
    private fun Project.configureSignTask() {
        tasks.withType<Sign>().configureEach {
            ...
            mustRunAfter(tasks.matching { it.name == "linkDebugTest$publicationName" })
            mustRunAfter(tasks.matching { it.name == "linkWorkerTestDebugTest$publicationName" })
            // Task ':compileTestKotlin<platform>' uses this output of task ':sign<platform>Publication' without declaring an explicit or implicit dependency
            mustRunAfter(tasks.matching { it.name == "compileTestKotlin$publicationName" })
        }
