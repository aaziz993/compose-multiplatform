...
import gradle.model.kotlin.kmp.jvm.android.MultipleVariantsSettings
import gradle.model.kotlin.kmp.jvm.android.Publishing
 ...
 */
@Serializable
internal data class LibraryPublishing(
    override val singleVariants: List<LibrarySingleVariant>? = null,
    /**
     * Publish multiple variants to a component.
     */
    val multipleVariants: List<MultipleVariantsSettings>? = null,
) : Publishing<LibrarySingleVariant> {

    override fun applyTo(publishing: com.android.build.api.dsl.Publishing<*>) {
        super.applyTo(publishing)

        publishing as LibraryPublishing

        multipleVariants?.forEach { multipleVariants ->
            multipleVariants.componentName?.let { componentName ->
                publishing.multipleVariants(componentName, multipleVariants::applyTo)
            } ?: publishing.multipleVariants(multipleVariants::applyTo)
        }
    }
}
