...
//    private val positiveNumber: Parser<Double> = ((INTEGER and optional(FLOATING_POINT_PART))
//        .map { (int, floatPart) ->
//            int.text + floatPart?.let { ".${it.text}" }.orEmpty()
//        } or
//        (ONLY_FLOATING_PART map { ".${it.text}" }) and
//        optional(EXPONENT_PART map { "e${it.text}" }))
//        .map { (p1, p2) ->
//            (p1 + (p2 ?: "")).toDouble()
//        }
//
//    private val NUMBER: Parser<Double> = (optional(MINUS) and positiveNumber)
//        .map { (m, num) -> Const(if (m != null) -num else num) }
//
