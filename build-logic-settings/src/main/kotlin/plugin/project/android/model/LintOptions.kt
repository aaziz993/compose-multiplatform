package plugin.project.android.model

import com.android.build.gradle.internal.dsl.LintOptions
import gradle.trySet
import kotlinx.serialization.Serializable
import org.gradle.api.Project

/**
 * Options for lint.
 */
@Serializable
internal data class LintOptions(
    /**
     * The set of issue id's to suppress.
     */
    val disable: Set<String>? = null,
    /**
     * The set of issue id's to enable.
     */
    val enable: Set<String>? = null,
    /** Issues that have severity overridden to 'informational' */
    val informational: Set<String>? = null,
    /** Issues that have severity overridden to 'warning' */
    val warning: Set<String>? = null,
    /** Issues that have severity overridden to 'error' */
    val error: Set<String>? = null,
    /** Issues that have severity overridden to 'fatal' */
    val fatal: Set<String>? = null,
    /**
     * The exact set of issues to check. If empty lint will detect the issues that are enabled
     * by default plus any issues enabled via [enable] and without issues disabled via [disable].
     */
    val checkOnly: Set<String>? = null,
    /** Whether lint should abort the build if errors are found  */
    val abortOnError: Boolean? = null,
    /**
     * Whether lint should display full paths in the error output. By default the paths
     * are relative to the path lint was invoked from.
     */
    val absolutePaths: Boolean? = null,
    /**
     * Whether lint should include the source lines in the output where errors occurred
     * (true by default)
     */
    val noLines: Boolean? = null,
    /**
     * Sets whether lint should be quiet (for example, not write informational messages
     * such as paths to report files written)
     */
    val quiet: Boolean? = null,
    /** Sets whether lint should check all warnings, including those off by default  */
    val checkAllWarnings: Boolean? = null,
    /** Sets whether lint will only check for errors (ignoring warnings)  */
    val ignoreWarnings: Boolean? = null,
    /** Sets whether lint should treat all warnings as errors  */
    val warningsAsErrors: Boolean? = null,
    /**
     * Sets whether lint should run all checks on test sources, instead of just the
     * lint checks that have been specifically written to include tests (e.g. checks
     * looking for specific test errors, or checks that need to consider testing code
     * such as the unused resource detector)
     */
    val checkTestSources: Boolean? = null,
    /**
     * Like [.isCheckTestSources], but always skips analyzing tests -- meaning that it also
     * ignores checks that have explicitly asked to look at test sources, such as the unused
     * resource check.
     */
    val ignoreTestSources: Boolean? = null,
    /**
     * Whether lint should ignore all testFixtures sources.
     */
    val ignoreTestFixturesSources: Boolean? = null,
    /**
     * Sets whether lint should run checks on generated sources.
     */
    val checkGeneratedSources: Boolean? = null,
    /** Sets whether lint should include explanations for issue errors. (Note that
     * HTML and XML reports intentionally do this unconditionally, ignoring this setting.)  */
    val explainIssues: Boolean? = null,
    /**
     * Sets whether lint should include all output (e.g. include all alternate
     * locations, not truncating long messages, etc.)
     */
    val showAll: Boolean? = null,
    /**
     * Sets an optional path to a lint.xml configuration file
     */
    val lintConfig: String? = null,
    /** Whether we should write an text report. Default false. The location can be
     * controlled by [.getTextOutput].  */
    val textReport: Boolean? = null,
    /**
     * The optional path to where a text report should be written. The special value
     * "stdout" can be used to point to standard output.
     */
    val textOutput: String? = null,
    /** Whether we should write an HTML report. Default true. The location can be
     * controlled by [.getHtmlOutput].  */
    val htmlReport: Boolean? = null,
    /** The optional path to where an HTML report should be written  */
    val htmlOutput: String? = null,
    /** Whether we should write an XML report. Default true. The location can be
     * controlled by [.getXmlOutput].  */
    val xmlReport: Boolean? = null,
    /** The optional path to where an XML report should be written  */
    val xmlOutput: String? = null,
    /**
     * Whether we should write a SARIF (OASIS Static Analysis Results Interchange Format) report.
     * Default is false. The location can be controlled by [sarifOutput].
     */
    val sarifReport: Boolean? = null,
    /**
     * The optional path to where a SARIF report (OASIS Static
     * Analysis Results Interchange Format) should be written.
     * Setting this property will also turn on [sarifOutput].
     */
    val sarifOutput: String? = null,
    /**
     * Sets whether lint should check for fatal errors during release builds. Default is true.
     * If issues with severity "fatal" are found, the release build is aborted.
     */
    val checkReleaseBuilds: Boolean? = null,
    /**
     * Sets whether lint should check all dependencies too as part of its analysis. Default is
     * false.
     */
    val checkDependencies: Boolean? = null,
    /**
     * Sets the baseline file to use, if any. The baseline file is
     * an XML report previously created by lint, and any warnings and
     * errors listed in that report will be ignored from analysis.
     *
     *
     * If you have a project with a large number of existing warnings,
     * this lets you set a baseline and only see newly introduced warnings
     * until you get a chance to go back and address the "technical debt"
     * of the earlier warnings.
     */
    val baseline: String? = null,
    /**
     * The target sdk version used when generating a lint report for a library.
     * This should only be set for libraries.
     */
//    val targetSdk: ApiVersion?=null,
) {

    context(Project)
    fun applyTo(options: LintOptions) {
        disable?.let(options.disable::addAll)
        enable?.let(options.enable::addAll)

        informational?.let { informational ->
            options.informational(*informational.toTypedArray())
        }

        warning?.let { warning ->
            options.warning(*warning.toTypedArray())
        }

        error?.let { error ->
            options.error(*error.toTypedArray())
        }

        fatal?.let { fatal ->
            options.fatal(*fatal.toTypedArray())
        }

        checkOnly?.let(options.checkOnly::addAll)
        options::isAbortOnError trySet abortOnError
        options::isAbsolutePaths trySet absolutePaths
        options::isNoLines trySet noLines
        options::isQuiet trySet quiet
        options::isCheckAllWarnings trySet checkAllWarnings
        options::isIgnoreWarnings trySet ignoreWarnings
        options::isWarningsAsErrors trySet warningsAsErrors
        options::isCheckTestSources trySet checkTestSources
        options::isIgnoreTestSources trySet ignoreTestSources
        options::isIgnoreTestSources trySet ignoreTestFixturesSources
        options::isCheckGeneratedSources trySet checkGeneratedSources
        options::isExplainIssues trySet explainIssues
        options::isShowAll trySet showAll
        options::lintConfig trySet lintConfig?.let(::file)
        options::textReport trySet textReport
        textOutput?.let(options::textOutput)
        options::htmlReport trySet htmlReport
        options::htmlOutput trySet htmlOutput?.let(::file)
        options::xmlReport trySet xmlReport
        options::xmlOutput trySet xmlOutput?.let(::file)
        options::sarifReport trySet sarifReport
        options::sarifOutput trySet sarifOutput?.let(::file)
        options::isCheckReleaseBuilds trySet checkReleaseBuilds
        options::isCheckDependencies trySet checkDependencies
        options::baselineFile trySet baseline?.let(::file)
    }
}
